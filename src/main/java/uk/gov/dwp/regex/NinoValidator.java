...
public class NinoValidator {
  ...
  /**
   * Calls setNino in order to maintain validation. If the validation fails throws an
  ...
  /**
   * Validates the input against Nino formatting. Checks carried out include: input is not null
   * input is not less than 9 characters input matches REGEX nino format
   *
   * <p>Called by static functions and constructor(String)
   *
   * @param ninoInput data number input to use
   * @return boolean Nino is valid? T/F
   */
  public static boolean validateNINO(String ninoInput) {
    return null != ninoInput
        && ninoInput.length() >= 9
        && reformatInput(ninoInput).matches(NINO_REGEX);
  }

  /**
   * Validates the input against stricter nino formatting rules (see CV0013). Checks carried out
   * include:
   *
   * <p>input is not null input is 9 characters exactly input matches REGEX nino format
   *
   * <p>The rules as written from the corporate data standards document (reference CV0013) are: +
   * The NI Number must be 9 characters long with no spaces + The first two characters must be
   * alphabetical + Characters three to eight must be numeric + Character nine must be A, B, C, D,
   * or a space + The first character must not be D, F, I, Q, U, or V + The second character must
   * not be D, F, I, O, Q, U, or V + The first two characters must not be combinations of GB, NK,
   * TN, or ZZ (this includes the reverse of each combination, so BG, KN, and NT are also not valid)
   *
  ...
  /**
   * Validates Nino input loosely, but returns a strict nino back which follows the corporate data
   * standards for a Nino.
   *
   * @param inputNino data number input to use
   ...
   *
   * <p>Called by: getDayOfWeek getFormNino validateNINO setNino strictReformatInput
   *
   * @param inputNino data number input to use
  ...
  private static String strictReformatInput(String inputNino) {
    return Optional.ofNullable(reformatInput(inputNino))
        .map(input -> input.length() < 9 ? input.concat(" ") : input)
        .orElse(null);
  }
  ...
  /**
   * Validates the Nino for the correct format. Returns true if the format is correct and data is
   * set, false otherwise.
   *
   ...
   *
   * @return String The main body of the Nino without the suffix. Nino should be unique without the
   *     suffix
   */
